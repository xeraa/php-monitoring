...
resource "aws_lightsail_key_pair" "php_monitoring_key_pair" {
  name       = "php_monitoring_key_pair"
...
resource "aws_lightsail_instance" "backend" {
  ...
  bundle_id         = "${var.size}"
  key_pair_name     = "php_monitoring_key_pair"
  depends_on        = ["aws_lightsail_key_pair.php_monitoring_key_pair"]
}
